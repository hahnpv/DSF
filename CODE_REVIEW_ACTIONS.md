...
- **A8 / A42** (Monte Carlo stats fiction + mc_cli dedup) — needs the numpy pre-draws to be transmitted into each C++ case (or read back). Non-trivial dispersion-plumbing rework; left as-is to avoid a wrong partial fix.
...
- **A37** (bind events/Monte Carlo/phase sequencer) — sizeable new binding surface; left for a dedicated pass.
...
- **A43/A44 tracked artifacts** — `.gitignore` updated, but committed `python/dsf/libsixdof.so.1*`, `*.egg-info/`, root `test/` verify-scripts, and `python/dsf/gui_tests/` are *tracked*; removing them needs `git rm` (left for you — no destructive git actions taken).
...
- [ ] **A32. Remove `-fpermissive` via one `const` overload.** Add `const Vec3& operator[](int) const` to `Mat3`; delete `-fpermissive` from all six targets. `DSF/util/math/mat3.h:68`, all `CMakeLists.txt` — *Accept:* full build succeeds with no `-fpermissive`.
...
- [ ] **A34. Single extension copy.** Make `dsf run` and `import dsf` load the same `.so` (install it, or load from one canonical location). `python/dsf/cli/run.py:11-38`, `python/dsf/__init__.py` — *Accept:* one `dsf_core` module object per process.
- [ ] **A35. dlopen-flag and .so-glob robustness.** Save/restore `sys.setdlopenflags`; select the `.so` by exact ABI tag, not first-glob. `python/dsf/cli/run.py:11-27` — *Accept:* importing h5py after `dsf.cli.run` doesn't change its flags; correct `.so` picked when two ABIs coexist.
- [ ] **A36. Move import-time `sys.exit`/side-effects out of module scope** in run.py into `main()`. `python/dsf/cli/run.py:35-38` — *Accept:* `import dsf.cli.run` doesn't kill the process.
- [ ] **A37. Bind `<events>` / Monte Carlo / phase sequencer** (or route `dsf run` through the C++ loader) so the Python path matches `examples/dynamic/main.cpp`. `python/bindings/bindings_sim.cpp`, `python/dsf/cli/run.py` — *Accept:* an event-terminated deck ends at the event via `dsf run`.
- [ ] **A38. CMake cleanup.** Give `util` PIC; stop double-compiling sim/util sources into `libDSF.so`; reconcile the drifted source lists; make pybind11 discoverable offline. `CMakeLists.txt:58-79`, `DSF/util/CMakeLists.txt` — *Accept:* clean build, no duplicate-symbol / relocation warnings.
- [ ] **A39. Test suite runs on a fresh clone.** De-symlink fixtures (commit real files under `fixtures/`); track `pytest.ini`; `pytest.skip` when sixdof is absent. `python/tests/fixtures/`, `pytest.ini`, `python/tests/conftest.py` — *Accept:* `git clone` + `pytest -m "not slow"` collects and runs (skipping sixdof tests) without errors.

---

## P3 — Duplication / dead code / docs / hygiene

- [ ] **A40. Consolidate the three interpolation implementations** into one shared bracket+interp helper. `DSF/util/tbl/{tbl,tbl2d,tablend}.cpp`
- [ ] **A41. Delete `server_for_dumb_models_untested.py`** (stale, broken, no path confinement). `python/dsf/mcp/`
- [ ] **A42. De-duplicate MC stats** (`mc_cli.py` should call `MonteCarlo` methods, not reimplement). `python/dsf/cli/mc_cli.py` vs `python/dsf/mc.py`
- [ ] **A43. Remove committed binaries/artifacts.** `python/dsf/libsixdof.so.1*`, `*.egg-info/`, stray `python/dsf.py`, root `test/` outputs. Update `.gitignore` (drop repo-wide `*.txt`/`*.csv`; add `*.egg-info/`, `/output/`, scratch dirs).
- [ ] **A44. Delete/port dead tests & examples.** `python/dsf/gui_tests/` (broken), root `test/` verify-scripts, `tests_example.cmake` (unwired), `examples/visualization/*` (moved-module import). Wire `test/test_tablend.cpp` into CTest (it now guards A2/A21).
- [ ] **A45. Correct CLAUDE.md gotchas.** XML parser is Boost Property Tree, not hand-rolled (residual whitespace bug is in *value* parsing); note `.dsf`-directory status; `build/sixdof_build` no longer created. `CLAUDE.md`
- [ ] **A46. Add C++ unit tests** for integrators (RK4/RK45/Verlet), tables, XML value parsing, events, and the factory — the currently-untested code that this review found broken.
