...
def run_map(h5_path: str):
    ...
    # The sixdof models log Latitude/Earth Longitude already in RADIANS (verified
    # against 6DOF.cpp / HydroEOM.cpp / 3DOF.cpp, units "rad", conversion 1.0), so
    # no degrees→radians conversion is applied here (that divided values by 57.3).
    ...
    for i, t in enumerate(times):
        ...
        for block_id, props in data.items():
            ...
            for prop, vals in props.items():
                ...
                block_frame[prop] = v
                # Detect lat/lon — already stored in radians in HDF5
                if prop in LAT_KEYS and vals.ndim == 1:
                    lat_val = float(vals[i])
                if prop in LON_KEYS and vals.ndim == 1:
                    lon_val = float(vals[i])
