...
class TestConstants:
    def test_pi(self):
        ...
        assert abs(float(out.strip()) - math.pi) < 1e-12

    def test_rad(self):
        out = _assert_runs("print(dsf.RAD)")
        # RAD converts radians → degrees, so 180/pi
        assert abs(float(out.strip()) - (180.0 / math.pi)) < 1e-10
...
class TestVec3:
    ...
    def test_dot(self):
        out = _assert_runs("""
            ...
            print(v1.dot(v2))
        """)
        assert abs(float(out.strip())) < 1e-12

    def test_cross(self):
        out = _assert_runs("""
            v = dsf.Vec3(1, 0, 0).cross(dsf.Vec3(0, 1, 0))
            print(v.x, v.y, v.z)
        """)
        x, y, z = map(float, out.split())
        assert abs(x) < 1e-12 and abs(y) < 1e-12 and abs(z - 1.0) < 1e-12
...
class TestTable:
    def test_1d_interp(self, tmp_path):
        ...
        out = _assert_runs(f"""
            ...
            print(t(1.5))
        """)
        assert abs(float(out.strip()) - 15.0) < 1e-10
    ...
    def test_1d_extrapolation_low(self, tmp_path):
        ...
        out = _assert_runs(f"""
            ...
            print(t(0.0))
        """)
        assert abs(float(out.strip())) < 1e-10
    ...
    def test_2d_interp_vec(self, tmp_path):
        ...
        out = _assert_runs(f"""
            ...
            print(vals[0], vals[1])
        """)
        v0, v1 = map(float, out.split())
        assert abs(v0 - 10.0) < 1e-6
        assert abs(v1 - 110.0) < 1e-6
